...
cost = [[sys.maxsize for _ in range(len(data[0]) * 5)] for _ in range(len(data) * 5)]
cost[0][0] = 0
visited = {}


def make_data_big():
    data_big = [[0 for _ in range(len(data[0]) * 5)] for _ in range(len(data) * 5)]
    for i in range(len(data)):
        for j in range(len(data[0])):
            for k in range(1, 6):
                to_place = data[i][j] + (k - 1)
                to_place = to_place - 9 if to_place > 9 else to_place

                data_big[i * k][j * k] = to_place
    print(data_big)
